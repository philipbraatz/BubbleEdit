...
//! [1]
    ...
    DragLabel* wordLabel = new DragLabel("First one", this);
    wordLabel->move(x, y);
    wordLabel->show();
    ...
    while (!inputStream.atEnd()) {
        ...
        if (!word.isEmpty()) {
            m_labelText = word;
            wordLabel = new DragLabel(m_labelText, this);
            wordLabel->move(x, y);
            wordLabel->show();
            wordLabel->setAttribute(Qt::WA_DeleteOnClose);
            x += wordLabel->width() + 2;
    ...
    wordLabel = new DragLabel("Last one", this);
    wordLabel->move(x, y);
    wordLabel->show();
    wordLabel->setAttribute(Qt::WA_DeleteOnClose);
    x += wordLabel->width() + 2;
...
//! [10]
//! [11]
        DragLabel *newLabel = new DragLabel(m_labelText, this);
